...
computeWT <- function(X, Y, A) {
  ...
  for (i in seq(A)) {
    ...
    # Compute weight matrix out <- eigen(AA) #well defined eigenvalue problem
    ...
    # Orthogonal projection matrix of scores Q[[i+1]] <- diag(N) - r[[i+1]] %*%
    # t(r[[i+1]])/ (t(r[[i+1]]) %*% r[[i+1]])[1]
    Q[[i + 1]] <- diag(N) - r[[i + 1]] %*% solve(t(r[[i + 1]]) %*% r[[i + 1]]) %*% t(r[[i +
                                                                                          1]])

    # Deflation step X-deflation step
  ...
  return(list(W = W, T_score = T_score, R = R))
}
