#' @title FM test
#' @description Performs permutation-based test based on FM
#' @usage FMTest(X, Y, nperm = 200, A, randomization = FALSE,
...
#' \code{\link{specificityTest}}, \code{\link{F1Test}}.
#' @author Angela Andreella
...
FMTest <- function(X, Y, nperm = 200, A, randomization = FALSE, Y.prob = FALSE, eps = 0.01,
  ...
  # FM observed
  if (cross.validation) {
    FM_obs <- repeatedCV_test(data = X, labels = Y, A = A, test_type = "FMTest", ...)
  } else {
    FM_obs <- FM(confMatrix = confMatrix)
  }
  ...
  if (randomization) {
    ...
    null_distr <- c(FM_obs, null_distr)
    pv <- mean(null_distr >= FM_obs)
  ...
  return(list(pv = pv, pv_adj = min(pv * A, 1), test = FM_obs))
}
